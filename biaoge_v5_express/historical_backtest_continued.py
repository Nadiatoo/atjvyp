...
class HistoricalBacktester:
    ...
    def _generate_analysis(self, node: HistoricalNode, predicted_state: str, match_score: float) -> str:
        ...
        if match_score >= 0.9:
            ...
            analysis = f"⚠️ 基本匹配：预期{expected_chinese}，预测{predicted_chinese}"
        elif match_score >= 0.5:
            analysis = f"⚠️ 部分匹配：预期{expected_chinese}，预测{predicted_chinese}"
        else:
            analysis = f"❌ 匹配度低：预期{expected_chinese}，预测{predicted_chinese}"
    ...
    def generate_report(self, results: List[BacktestResult]) -> Dict[str, Any]:
        """生成回测报告"""
        ...
        perfect_accuracy = perfect_matches / total_nodes * 100 if total_nodes > 0 else 0
        good_accuracy = good_matches / total_nodes * 100 if total_nodes > 0 else 0
        
        # 计算平均置信度
        avg_confidence = np.mean([r.confidence for r in results]) * 100 if results else