...
class StateJumpEngine:
    ...
    def _record_jump_history(self, current_state: str, prediction: StateJumpPrediction, active_factors: List[JumpFactor]):
        ...
        history_entry = {
            ...
            "from_state": current_state,
            "to_state": prediction.to_state,
            "probability": prediction.probability,
            "confidence": prediction.confidence,
            "risk_level": prediction.risk_level,
            "trigger_factor": prediction.trigger_factor.description,
            "recommended_action": prediction.recommended_action,
            "active_factors_count": len(active_factors)
        }
        
        self.jump_history.append(history_entry)
        
        # 保持历史记录不超过100条
        if len(self.jump_history) > 100:
            self.jump_history = self.jump_history[-100:]
        
        logger.debug(f"记录状态跳跃历史: {current_state} → {prediction.to_state}")
    
    def get_jump_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        """获取跳跃历史"""
        return self.jump_history[-limit:] if self.jump_history else []
    
    def get_risk_summary(self) -> Dict[str, Any]:
        """获取风险摘要"""
        if not self.jump_history:
            return {"total_jumps": 0, "average_risk": "low"}
        ...
        total_risk = sum(risk_scores.get(h["risk_level"], 1) for h in self.jump_history)
        avg_risk_score = total_risk / len(self.jump_history)
        ...
        return {
            ...
            "last_jump": self.jump_history[-1] if self.jump_history else None,
            "high_risk_jumps": len([h for h in self.jump_history if h["risk_level"] in ["high", "extreme"]])
        }
...
def quick_test():
    ...
    factor = JumpFactor(
        ...
        source="test",
        timestamp=datetime.now(),
        policy_type="monetary_easing"
    )
    
    predictions = engine.predict_state_jumps("winter_hiding", [factor])
    
    if predictions:
        print(f"预测到 {len(predictions)} 个可能的跳跃")
        for pred in predictions[:2]:
            print(f"  • {pred.from_state} → {pred.to_state} (概率: {pred.probability*100:.1f}%)")
