...
def get_egps_analysis():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            analysis_results["economic_cycle"] = {"error": "获取失败"}
        ...
        if response.status_code == 200:
            ...
            analysis_results["policy_environment"] = {"error": "获取失败"}
        ...
        if response.status_code == 200:
            ...
            analysis_results["system_status"] = {"status": "未知"}
...
def format_simple_message(analysis_results):
    """格式化简单消息"""
    print("格式化飞书消息...")
...
def main():
    ...
    if time_diff > 1800:  # 30分钟
        print(f"不在08:00推送时间窗口内（时间差: {time_diff/60:.1f}分钟）")
        return
    
    print("✅ 符合推送条件（工作日08:00）")
    ...
    # 格式化消息
    message_text = format_simple_message(analysis_results)
    
    # 在OpenClaw环境中，消息会自动发送
    print("\n📱 飞书消息内容预览:")
    print("=" * 60)
    print(message_text)
    print("=" * 60)
    ...
    print("\n✅ EGPS分析报告已准备就绪")
    print("在OpenClaw环境中，此消息会自动推送到飞书")
